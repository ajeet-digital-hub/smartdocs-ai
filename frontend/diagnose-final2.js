/**
 * Final diagnostic v2: Google control test + nss certificate check
...
async function main() {
  ...
  const certStrings = rootCerts.map(c => typeof c === 'string' ? c : (c && c.toString ? c.toString() : JSON.stringify(c)));

  const nssCert = certStrings.find(cert => cert.includes('CN=nss'));
  if (nssCert) {
  ...
  // Check for non-standard CAs
  console.log('\n[Root CAs] Non-standard root CAs (not from common public CAs):');
  const standardIssuers = ['DigiCert', 'GlobalSign', 'Sectigo', 'Let\'s Encrypt', 'GoDaddy', 'Entrust', 'VeriSign', 'Comodo', 'Atos', 'Amazon', 'Google', 'Microsoft', 'ISRG', 'Starfield', 'AddTrust', 'USERTRUST', 'AAA Certificate', 'Security Communication'];
  const nonStandard = certStrings.filter(cert => {
  ...
  console.log('\n=== Summary ===');
  console.log('Google TLS: ' + (r.ok ? 'OK (' + r.protocol + ')' : 'FAILED'));
  console.log('MongoDB TLS: FAILED with internal_error (alert 80) on ALL tests');
  console.log('');
